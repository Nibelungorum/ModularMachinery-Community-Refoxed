...
public class SmartInterfaceGameTest {
    ...
    public void bindsDefaultValueAndWritesRecipeOutput(GameTestHelper helper) {
        ...
        helper.assertTrue(smartInterface.bindingFor(controllerPos).map(binding -> binding.value() == 12F).orElse(false),
        ...
        helper.assertTrue(smartInterface.bindingFor(controllerPos).map(binding -> binding.value() == 42F).orElse(false),
