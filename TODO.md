...
设计和实施计划已完成。
文件：
- 设计规格：docs/superpowers/specs/2026-08-14-modular-machine-host-module-design.md
- 实施计划：docs/superpowers/plans/2026-08-14-modular-machine-host-module.md
计划包含：
- 机器角色与耦合器数量校验
- 模块耦合器方块实体及连接持久化
- 主机/模块结构匹配与接口冲突处理
- 事件触发、刷新队列和 60 tick 低频校验
- 模块配方主机门控
- 控制器 UI 状态同步
- JEI 多主机提示轮播
- KubeJS 声明 API 与动态重载回滚
- 预览隔离
- 单元测试、GameTest 和最终 Gradle 验证

- [ ] 可扩展型机器 或者说多层级机器 参考蒸馏塔 以及可丢失型合成

实施计划已写入：
docs/superpowers/plans/2026-08-14-expandable-structures-partial-outputs.md
计划包含 7 个可独立验证的任务：
1. 可扩展机器声明、结构阶段模型与合并校验
2. KubeJS 结构 API、运行期结构族与多阶段编译
3. 从最高阶段开始匹配、自动升级/降级及阶段状态持久化
4. 默认搭建最小结构、按当前阶段拆除
5. 配方级 allowPartialOutputs()、Codec 与 KubeJS API
6. 按声明顺序执行的物品/流体部分输出规划与原子提交
7. 蒸馏塔式端到端 GameTest 和完整验证

- [ ] 接口集成(AE2(这个得后面说了) 总成型接口[新UI?] MEK气体接口(涉及配方modify) 输入输出总成)
