...
Spec 和 implementation plan 已完成：
- 设计文档：docs/superpowers/specs/2026-08-13-recipe-locking-design.md
- 实现计划：docs/superpowers/plans/2026-08-13-recipe-locking.md
计划覆盖：
- FactoryRecipeThread 按线程保存锁定配方
- 普通控制器和多线程搜索限制
- 锁定失败时保持锁定并返回缺少输入
- 红石暂停时使用 pausedActive/pausedContext
- 服务端权威网络请求和权限校验
- 多线程快照及当前线程 UI 切换
- 原版正方形配方书按钮和 Tooltip
- 存档持久化、旧存档兼容、配方不存在时清除
- 单元测试、GameTest 和最终并行全量验证
...
为多方块机器添加一种新类型的结构，允许多方块机器在声明时声明自己为 主控型结构，即，该多方块机器为其它多方块机器提供插槽，其它多方块机器在插槽位置安装后，当主控型机器成型，才允许插槽型机器运行配方，同时，在主控的控制器UI渲染: 安装的模块数量: xx，在插槽机器的控制器UI渲染，已安装到xxx(这个是主控的名称)，未安装到主控则显示 xxx 未连接。
